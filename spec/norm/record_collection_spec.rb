...
module Norm
  describe RecordCollection do
    ...
    describe 'initialization' do
      ...
      it 'requires a record class if no records given' do
        proc { RecordCollection.new }.must_raise ArgumentError
        RecordCollection.new(record_class: person_record_class).
          must_be_kind_of RecordCollection
      end

      it 'is empty when no records are supplied' do
        RecordCollection.new(record_class: person_record_class).must_be :empty?
      end

      it 'has one record when a single record is supplied' do
        RecordCollection.new(ernie).size.must_equal 1
