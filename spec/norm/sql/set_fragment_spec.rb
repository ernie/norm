...
module Norm
  module SQL
    describe SetFragment do
      ...
      it 'builds sql/params with equality from a hash' do
        fragment = subject.new(:id => 1)
        fragment.sql.must_equal '"id" = $?'
        fragment.params.must_equal [1]
      end

      it 'builds sql/params with "= NULL" if a nil is on RHS of hash' do
        fragment = subject.new(:id => nil)
        fragment.sql.must_equal '"id" = NULL'
